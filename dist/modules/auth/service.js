...
import { generateAccessToken, generateRefreshToken, verifyRefreshToken, } from "../../shared/utils/tokenHandler.js";
...
export class AuthService {
    ...
    async registerInitiate(input) {
        ...
        if (email) {
            otpTasks.push(saveOtp("email", email, emailOtp), sendEmailOtp(email, emailOtp));
        }
    ...
    async registerVerify(input) {
        ...
        await this.repo.createRefreshToken(user.id, refreshToken, new Date(Date.now() + SEVEN_DAYS_MS));
        await this.repo.clearPendingRegistration(phoneNumber);
        return { user, accessToken, refreshToken };
    }
    // ── Login Step 1: verify password + send OTP ─────────────────────────────
    async loginInitiate(input) {
        ...
        if (!user.passwordHash) {
            throw new AppError("No web password set. Please register on the web to activate web login.", 403);
        }
    ...
    async refresh(token) {
        ...
        await this.repo.rotateRefreshToken(token, user.id, newRefreshToken, new Date(Date.now() + SEVEN_DAYS_MS));
