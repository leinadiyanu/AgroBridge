...
export class OrderService {
    constructor(repo, listingRepo) {
        this.repo = repo;
        this.listingRepo = listingRepo;
    }
    async placeOrder(buyerId, userRole, data) {
        ...
        if (data.deliveryMethod !== "SELF_PICKUP") {
            if (!data.deliveryLocation) {
                throw new AppError("Delivery location is required for this delivery method", 400);
            }
            const quote = await logisticsService.getDeliveryQuote(listing.location, data.deliveryLocation);
            deliveryDistance = quote.distance;
            deliveryFee = quote.fee;
        }
        const order = await this.repo.create(buyerId, listing.farmerId, data, totalPrice, deliveryDistance, deliveryFee);
    ...
    async getMyOrders(userId, userRole, filters) {
    ...
    async getOrder(userId, userRole, orderId) {
    ...
    async confirmOrder(userId, userRole, orderId) {
        if (userRole !== Role.FARMER) {
    ...
    async cancelOrder(userId, userRole, orderId) {
        const order = await this.repo.findById(orderId);
        if (!order)
            throw new AppError("Order not found", 404);
        const isInvolved = order.buyerId === userId || order.farmerId === userId;
        ...
        if (order.status === "PENDING" && order.farmerId === userId) {
            throw new AppError("Farmers cannot cancel pending orders — wait for it to be confirmed first", 400);
        }
        await notificationService.notify({
    ...
    async completeOrder(userId, userRole, orderId) {
        if (userRole !== Role.BUYER) {
