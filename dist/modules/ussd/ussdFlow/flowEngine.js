...
export class FlowEngine {
    ...
    async run(flow, steps, phoneNumber) {
        // 1. Load existing session from Redis (or start fresh)
        let session = (await this.repo.getSession(phoneNumber)) ?? {
            step: flow.start,
            data: {},
        };
        ...
        if (currentStep.action === "MY_INCOMING_ORDERS") {
            try {
                ...
                const lines = orders
                    .slice(0, 3)
                    .map((o, i) => `${i + 1}. ${o.listing.crop} ${o.quantity}kg\n   Buyer: ${o.buyer.name}`)
                    .join("\n");
        ...
        if (currentStep.action === "MY_LISTINGS") {
            try {
                ...
                const lines = listings
                    .slice(0, 3)
                    .map((l, i) => `${i + 1}. ${l.crop} ${l.quantity}kg @${l.price}/kg`)
                    .join("\n");
                // Save listing IDs in session for actions
                // session.data.listingIds = listings.slice(0, 3).map((l: any) => l.id);
                session.data.listingIds = listings.slice(0, 3).map((l) => l.id).join(",");
        ...
        if (currentStep.action === "BROWSE_PRODUCE") {
            try {
                const listings = await this.repo.getListingsByCrop(latestInput.trim());
                await this.repo.deleteSession(phoneNumber);
                if (!listings.length) {
                ...
                const lines = listings
                    .slice(0, 3)
                    .map((l) => `${l.crop} ${l.quantity}kg @${l.price}/kg - ${l.location}`)
                    .join("\n");
                return `END Results:\n${lines}`;
            }
            catch {
