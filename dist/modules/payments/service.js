...
export class PaymentService {
    ...
    async initiatePayment(buyerId, orderId) {
        ...
        if (!order.buyer.email) {
            throw new AppError("Email is required to process payment. Please add an email to your profile.", 400);
        }
        ...
        const paystackInit = await initializePaystackPayment(order.buyer.email, grandTotal, reference);
    ...
    async verifyPayment(reference) {
        ...
        try {
            ...
            await this.repo.updateEscrow(transaction.id, escrow.data.escrow_id, "held");
        }
        catch {
    ...
    async releasePayment(orderId) {
        ...
        await this.repo.updateEscrow(transaction.id, transaction.escrowRef, "released");
