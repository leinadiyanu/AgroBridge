...
export class PredictionService {
    ...
    async buildInput(commodity, state) {
        const now = new Date();
        ...
        const rolling_mean3 = prices.length > 0 ? prices.reduce((a, b) => a + b, 0) / prices.length : 0;
        return {
            commodity,
            state,
    ...
    async predict(input) {
        ...
        try {
            const { data } = await axios.post(`${ML_BASE_URL}/predict`, mlInput);
            return data;
        }
        catch {
            throw new AppError("Price prediction unavailable. Please try again later.", 503);
        }
    }
    // ── Best time to sell ──────────────────────────────────────────────────────
    async bestTime(commodity) {
        if (!commodity) {
            throw new AppError("Commodity is required", 400);
        }
        try {
            const { data } = await axios.get(`${ML_BASE_URL}/best-time/${encodeURIComponent(commodity)}`);
            return data;
        }
        catch {
            throw new AppError("Could not fetch best time to sell. Please try again later.", 503);
        }
    }
    // ── Health check ───────────────────────────────────────────────────────────
    async isHealthy() {
