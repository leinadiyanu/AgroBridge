...
8. Other`,
    ...
    error: "CON Invalid option. Select 1-8",
    transform: (input: string) => {
      ...
      return map[input] ?? "OTHER"; // "8" routes to a free-text fallback step
    },
    next: (input: string) => (input === "8" ? "postCropOther" : "postQuantity"),
  },
  postCropOther: {
  ...
  // },
  postDate: {
    prompt: "CON Available from\n1. Now\n2. Enter a date",
    validate: (input: string) => ["1", "2"].includes(input),
    error: "CON Select 1 or 2",
    next: (input: string) => (input === "1" ? "postDateNow" : "postDateCustom"),
  },
  postDateNow: {
  transform: () => new Date().toLocaleDateString("en-GB"),
  action: "POST_PRODUCE",
},
  postDateCustom: {
    prompt: "CON Available from (DD/MM/YYYY)",
